...
function Input({ className, type, ...props }: React.ComponentProps<"input">) {
  return (
    <input
      type={type}
      data-slot="input"
      // Password-manager/form-filler browser extensions (e.g. LastPass) tag
      // scanned inputs with a `fdprocessedid` attribute before React
      // hydrates, which otherwise trips a dev-only hydration-mismatch
      // warning for an attribute the app never sets or reads. Confirmed via
      // diffing the actual mismatch — this is the only attribute involved.
